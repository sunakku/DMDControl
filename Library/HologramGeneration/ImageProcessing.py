...
def smoothingfunc(gratingphase,alpha,omega,xsize,ysize):
    return  (np.tanh(alpha*(gratingphase+omega/2))-np.tanh(alpha*(gratingphase-omega/2)))/2  #probability
...
class TargetImage(object):
    ...
    def imagefft(self):
        if (self.image == "gaus"):
            ...
            self.fimage[0] = np.absolute(sinc(self.X,self.Y,self.width))**2#.24
            ...
            self.fimage[0] = 1*(self.X**2+self.Y**2 <100)+((self.X-100)**2+self.Y**2 <100)+((self.X+100)**2+self.Y**2 <100)
            ...
            self.fimage[0] = 1*(np.mod(self.X+self.Y,250) <20)
            self.fimage[0] *= gaus(self.X,self.Y,0.25)  
        elif (self.image == "lattice"):
            self.fimage[0] = 1*np.logical_or((np.mod(self.X+self.Y,100) <20),(np.mod(self.X-self.Y,100) <20)) *0.5
...
class Hologram(object):
    def __init__(self, **kwargs):
        ...
        self.xshift = kwargs.pop('xshift', 0)
        self.yshift = kwargs.pop('yshift', 0)
    ...
    def compute_hologram(self):
        ...
        phasesum = np.mod(-self.phasemap_large[1]+self.fimage[1]+self.X*self.xshift+self.Y*self.yshift,2.*math.pi)*12./(2.*math.pi)
...
def gaus(xx,yy,width):
    return 1*np.exp(-(xx**2+yy**2)/(2.*(width*xx.shape[0]*0.1*2)**2))
...
def gaussian(factor,phasemap):
    width=0.01*np.shape(phasemap)[1]
...
def hologramize2(X,Y,alpha,amp,phasesum,imageamp,amp2,phasesum2,imageamp2,resolution):
    dmdpattern = np.zeros(resolution)
    gratingphase = np.absolute(np.mod(Y-X+phasesum,12.)-6)/6.#np.mod(Y-X,12.)/12.#
    gratingphase2 = np.absolute(np.mod(Y-X+phasesum2,12.)-6)/6.#np.mod(Y-X,12.)/12.#
    #####set minimum value for amp.
    #####to avoid inf in omega
    #####effect to intensity:1%
    #####should find way around this problem
    ...
    amp2 += 0.01
    omega = np.divide(imageamp,amp)/np.amax(np.divide(imageamp,amp))#imageamp#
    omega2 = np.divide(imageamp2,amp2)/np.amax(np.divide(imageamp2,amp2))#imageamp#
    Xsize,Ysize = np.shape(amp)

    prob = rand_smoothing(gratingphase,alpha,omega,Xsize,Ysize)
    prob2 = rand_smoothing(gratingphase2,alpha,omega2,Xsize,Ysize)
    #hologram=1*(prob+prob2 <2)
    hologram=1*np.logical_or((prob<1),prob2<1)
    dmdpattern[540-Xsize/2:540+Xsize/2,960-Ysize/2:960+Ysize/2]=hologram
    plt.figure(2,(10,5))
    plt.subplot(2,1,1)
    plt.imshow(1*(prob<1))
...
def holograms(X,Y,alpha,amp,phasesum,imageamp,resolution):
    iteration = amp.shape[0]
    gratingphase = np.zeros(amp[0].shape)
    omega = np.zeros(amp[0].shape)
    prob = np.zeros(amp[0].shape)
    #prob = np.zeros((iteration,amp[0].shape[1],amp[0].shape[1]))
    dmdpattern = np.zeros(resolution)
    Xsize,Ysize = np.shape(amp[0])
    

    for i in xrange(iteration):
        gratingphase += np.absolute(np.mod(Y-X+phasesum[i],12.)-6)/6.
        amp[i] += 0.01
        omega += np.divide(imageamp[i],amp[i])/np.amax(np.divide(imageamp[i],amp[i]))#imageamp#
        #prob+= smoothingfunc(gratingphase,alpha,omega,Xsize,Ysize)

    #hologram=1*(prob<iteration)
    #hologram = np.zeros(amp[0].shape)
    #hologram = 1*(np.divide(np.random.rand(Xsize,Ysize),prob<1)
    hologram = 1*(rand_smoothing(gratingphase/(iteration*1.),alpha,omega/(iteration*1.),Xsize,Ysize)<2.3)
    print iteration

    
    #for i in xrange(iteration):
    #    hologram = np.logical_or(hologram,(prob[i]<1))
    dmdpattern[540-Xsize/2:540+Xsize/2,960-Ysize/2:960+Ysize/2]=hologram
    return dmdpattern
